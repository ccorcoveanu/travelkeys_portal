<section class="featured__item -is-special">
    <div class="featured__picture__container">
        {if $featured_item->image|strlen}
            <img class="destination__image" src="{$smarty.const.CDN}/{$featured_item->image_m2}" alt="{$featured_item->name}"/>
        {else}
            <img class="destination__image" src="{$smarty.const.TEMPLATE_PATH}/assets/images/placeholders/villa6.jpg" alt="{$featured_item->name}"/>
        {/if}
    </div>
    <div class="featured__wrapper">
        <div class="featured__item__wrapper">
            <h3 class="featured__item__title">
                <a href="#" title="Beverly Hills Mansion"><i class="icon icon__heart-featured"></i><span>Beverly Hills Mansion</span></a>
            </h3>
