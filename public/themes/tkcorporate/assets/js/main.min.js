...
document.getElementById("ck-city").checked&&amenities.push("ck-city");var filters={amenities:amenities,reservations:{start:$('input[name="startdate"]').val(),end:$('input[name="enddate"]').val()},group:{all:document.getElementById("ck-all").checked,specials:document.getElementById("ck-sp").checked,favorites:document.getElementById("ck-fav").checked}};load($('input[name="q"]').val(),start,20,filters)})}return __init()},filterSearch:function(){function __init(){if(!$(".search-results").length)return!1;var loading=!1,xhr_req=!1;$(".filter__input, .filter__option input").on("change",function(evt){window.scrollTo(0,0),$("#load-more__display").slideDown();var amenities=[];document.getElementById("ck-beach").checked&&amenities.push("ck-beach"),document.getElementById("ck-city").checked&&amenities.push("ck-city");var filters={amenities:amenities,reservations:{start:$('input[name="startdate"]').val(),end:$('input[name="enddate"]').val()},group:{all:document.getElementById("ck-all").checked,specials:document.getElementById("ck-sp").checked,favorites:document.getElementById("ck-fav").checked},guests:$("#guests__filter--item").val(),bedrooms:$("#bedroom__filter--item").val(),price:{start:$("#range-slider__low").val(),end:$("#range-slider__high").val()},order:$(".order-villas__select:visible select option:selected").val()};loading&&xhr_req&&xhr_req.abort(),xhr_req=$.getJSON("/ajax/filter",{q:$('input[name="q"]').val(),start:0,limit:20,filters:filters}).done(function(data){"ok"===data.status&&($(".featured__row__container .featured__row").html(data.html),$("#total_villas_number").text(data.total_items)),data.length<20&&$(".button_container_loadmore").hide(),$("#load-more__display").slideUp()}).fail(function(jqxhr,textStatus,error){}).always(function(){loading=!1}),loading=!0})}return __init()}};jQuery(document).ready(function(){travelkeys.init()}),jQuery(window).load(function(){});
